...
async function main() {
  ...
  if (!movie) {
    console.log('Movie not found in MovieOverride');
    return;
  }

  console.log('Current awards count:', movie.awards.length);

  const mubiData = await fetchMubiAwards(tmdbId, 'Diaz - Non pulire questo sangue', 'Diaz', '2012');
  if (!mubiData) {
  ...
  console.log('Updated awards in DB successfully.');
}
